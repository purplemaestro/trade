...
def recommend_swing_trade(json_data):
    ...
    return sorted(results, key=lambda x: (x["score"], x["p3m"], x["pch"]), reverse=True)
...
def calculate_roe(details):
    pat = details.get("pat", 0)
    sh = details.get("sh", 0)       # shares outstanding
    eps = details.get("eps", 0)

    if pat <= 0 or sh <= 0 or eps <= 0:
        return None

    # Equity = PAT / (ROE%) → but we don’t have direct
    # Instead: Book Value per Share = EPS / (ROE%)
    # Rearranged: ROE = EPS / (Book Value per Share)

    # Approximate book value per share
    equity = details.get("as", 0)   # fallback: assets if equity not available
    if equity <= 0:
        return None

    bvps = equity / sh  # book value per share
    if bvps <= 0:
        return None
